#!/usr/bin/env bash
# reproduction: Vector Retrieval Score Collapse (BUG-016)
# Verifies that self-similarity queries return scores > 0.7 after fix.
#
# Requires: KnowWhere server running on localhost:3737 with Ollama backend

set -euo pipefail
BASE_URL="${KNOWWHERE_URL:-http://localhost:3737}"
PASS=0
FAIL=0

red()   { echo -e "\033[31m$*\033[0m"; }
green() { echo -e "\033[32m$*\033[0m"; }

assert_score_gt() {
  local label="$1" query="$2" threshold="$3"
  local resp score
  resp=$(curl -s -X POST "$BASE_URL/retrieve_fractal" \
    -H 'Content-Type: application/json' \
    -d "{\"query_text\":\"$query\",\"top_k\":3,\"include_debug\":true}" 2>&1)
  score=$(echo "$resp" | jq -r '.[0].score // "error"' 2>/dev/null || echo "parse_error")

  if [ "$score" = "error" ] || [ "$score" = "parse_error" ] || [ "$score" = "null" ]; then
    red "  ✗ $label: no results (response: $(echo "$resp" | head -c 200))"
    FAIL=$((FAIL + 1))
  elif awk "BEGIN {exit !($score >= $threshold)}" 2>/dev/null; then
    green "  ✓ $label: score=$score (>= $threshold)"
    PASS=$((PASS + 1))
  else
    red "  ✗ $label: score=$score (< $threshold) — PREFIX BUG REGRESSION?"
    FAIL=$((FAIL + 1))
  fi
}

echo "=== BUG-016 Vector Retrieval Regression Tests ==="
echo "Server: $BASE_URL"
echo ""

# Ensure at least one knowwhere node exists for queries to match
echo "--- Setup: store test node ---"
curl -s -X POST "$BASE_URL/store_external" \
  -H 'Content-Type: application/json' \
  -d '{
    "pointer": "test://regression-016",
    "content": "KnowWhere ist ein fractales Memory-System mit Vektor-Retrieval und hybriden Search-Strategien",
    "memory_type": "semantic",
    "metadata": {"test_id": "bug-016-regression"}
  }' | jq -r '.message // .id // "stored"'
echo ""

echo "--- Core Test Cases ---"

# Test 1: Exact self-similarity (must score >0.7)
assert_score_gt \
  "Exact self-match" \
  "KnowWhere ist ein fractales Memory-System mit Vektor-Retrieval und hybriden Search-Strategien" \
  0.7

# Test 2: Close paraphrase
assert_score_gt \
  "Close paraphrase" \
  "Was ist KnowWhere für ein Speichersystem?" \
  0.4

# Test 3: Single keyword (edge: short query)
assert_score_gt \
  "Short query (1 word)" \
  "KnowWhere" \
  0.3

# Test 4: Query with special characters
assert_score_gt \
  "Special chars" \
  "KnowWhere: Vektor-Retrieval & hybrid Search — was ist das?" \
  0.5

# Test 5: German language (primary corpus language)
assert_score_gt \
  "German query" \
  "Wie funktioniert die Suche in KnowWhere?" \
  0.4

echo ""
echo "--- Edge Cases ---"

# Edge 1: Empty query (should error, not crash)
echo -n "  Empty query: "
resp=$(curl -s -X POST "$BASE_URL/retrieve_fractal" \
  -H 'Content-Type: application/json' \
  -d '{"query_text":"","top_k":3}' 2>&1)
if echo "$resp" | jq -e '.detail // .message' 2>/dev/null | grep -qi "empty"; then
  green "✓ correctly rejected empty query"
  PASS=$((PASS + 1))
else
  red "✗ empty query should be rejected (got: $(echo "$resp" | head -c 100))"
  FAIL=$((FAIL + 1))
fi

# Edge 2: Very long query (500+ chars)
LONG_QUERY="KnowWhere ist ein fortschrittliches Memory-System, das speziell für AI-Agenten entwickelt wurde. $(python3 -c "print('Es verwendet fractale Datenstrukturen. ' * 30)")"
assert_score_gt "Long query (500+ chars)" "$LONG_QUERY" 0.3

# Edge 3: Unicode/emoji query
assert_score_gt "Unicode query" "🧠 KnowWhere Memory-System — wie funktioniert es?" 0.4

# Edge 4: Query with only stopwords (may get low scores, just verify no crash)
echo -n "  Stopword query: "
resp=$(curl -s -X POST "$BASE_URL/retrieve_fractal" \
  -H 'Content-Type: application/json' \
  -d '{"query_text":"der die das und oder","top_k":3,"include_debug":true}' 2>&1)
score=$(echo "$resp" | jq -r '.[0].score // "no_results"' 2>/dev/null)
if [ "$score" != "parse_error" ]; then
  green "  ✓ stopword query returned without crash (score=$score)"
  PASS=$((PASS + 1))
else
  red "  ✗ stopword query caused parse error"
  FAIL=$((FAIL + 1))
fi

echo ""
echo "=== Results: $PASS passed, $FAIL failed ==="
if [ "$FAIL" -gt 0 ]; then
  exit 1
fi
