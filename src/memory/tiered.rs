...
impl TieredCompactionWorker {
    ...
    pub async fn compact_memory(
        ...
        match summary_result {
            Ok(result) => {
                ...
                self.store_tier_summary(memory_id, result.text, target)
                    .await?;
                Ok(memory_id)
            }
            Err(e) => {
    ...
    async fn store_tier_summary(
        ...
        let query = format!("UPDATE memories SET {} = $1 WHERE id = $2", column);
        sqlx::query(&query)
            .bind(&summary)
            .bind(memory_id)
            .execute(&self.pool)
    ...
    #[deprecated(
        since = "0.2.0",
        note = "Truncation causes information loss. Use LocalSummarizer or VLM."
    )]
