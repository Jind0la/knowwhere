...
/// Group consolidation candidates by session_id from metadata.
///
/// Looks up each candidate node via the store to read metadata["session_id"].
/// Returns groups sorted by turn_index within each session.
async fn group_by_session(
    candidates: &[(Uuid, DateTime<Utc>)],
    store: &dyn StorageBackend,
) -> HashMap<String, Vec<(Uuid, DateTime<Utc>)>> {
    let mut groups: HashMap<String, Vec<(Uuid, DateTime<Utc>)>> = HashMap::new();
    for (node_id, created_at) in candidates {
        if let Ok(Some(node)) = store.get(node_id).await {
            let session_id = node
                .metadata
                .get("session_id")
                .and_then(|v| v.as_str())
            ...
            groups
                .entry(session_id)
                .or_default()
                .push((*node_id, *created_at));
        }
    }
    // Sort each session's candidates by turn_index where available
    // (We can't sort here without the nodes, but the caller does it)
    groups
}
...
impl ConsolidationScheduler {
    ...
    pub async fn trigger_if_needed(self: &Arc<Self>) {
        // Allow disabling consolidation during benchmarks (Ollama thrashing)
        if std::env::var("KNOWWHERE_SKIP_CONSOLIDATION").is_ok() {
            return;
        }
    ...
    pub async fn pending_count(&self) -> (usize, usize) {
        ...
        let lenient: bool = std::env::var("KNOWWHERE_CONSOLIDATION_LENIENT")
            .map(|v| !v.eq_ignore_ascii_case("false") && v != "0")
            .unwrap_or(false);
        let min_importance: i32 = std::env::var("KNOWWHERE_CONSOLIDATION_MIN_IMPORTANCE")
            .ok()
        ...
        let candidates = all_nodes
            ...
            .filter(|n| {
                n.context_tier == ContextTier::Raw
                    ...
                    && (lenient || n.status == MemoryStatus::Active)
                    && (lenient || n.importance >= min_importance)
                    && (n
                        .content
                        .as_ref()
                        .map(|c| c.len() > min_content_len)
                        .unwrap_or(false)
                        || n.original_pointer
                            .as_ref()
                            .map(|p| p.len() > min_content_len)
                            .unwrap_or(false))
            })
            .count();
        (candidates, total)
    ...
    pub async fn force_run(&self) -> (usize, usize, u64) {
        ...
        for node in all_nodes {
            ...
            if !lenient && node.status != MemoryStatus::Active {
                continue;
            }
            if !lenient && node.importance < min_importance {
                continue;
            }
            let content_len = node
                ...
                .map(|c| c.len())
                .unwrap_or(0)
                .max(node.original_pointer.as_ref().map(|p| p.len()).unwrap_or(0));
    ...
    pub fn start_safety_net(self) -> (Arc<Self>, tokio::task::JoinHandle<()>) {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Skip if KNOWWHERE_SKIP_CONSOLIDATION is set (benchmark mode).
            if std::env::var("KNOWWHERE_SKIP_CONSOLIDATION").is_ok() {
                sched.is_running.store(false, Ordering::Release);
                loop {
    ...
    async fn run(&self) {
        ...
        // Session-level grouping: if KNOWWHERE_SESSION_CONSOLIDATION is set,
        // group candidates by session_id and log session summaries
        if std::env::var("KNOWWHERE_SESSION_CONSOLIDATION").is_ok() {
            let session_groups = group_by_session(&candidates, self.store.as_ref()).await;
            if !session_groups.is_empty() {
                let total_sessions = session_groups.len();
                let multi_node_sessions = session_groups.values().filter(|g| g.len() > 1).count();
                tracing::info!(
                    total_sessions,
                    multi_node_sessions,
                    total_candidates = candidates.len(),
                    "Session-level consolidation: grouped candidates"
                );
                for (sid, nodes) in &session_groups {
                    if nodes.len() > 1 {
                        tracing::debug!(
                            session_id = %sid,
                            node_count = nodes.len(),
                            "Multi-node session detected"
                        );
                    }
                }
            }
    ...
    async fn process_local_compaction(&self, node_id: Uuid) -> anyhow::Result<()> {
        ...
        let content = node.content.as_deref().unwrap_or("").to_string();
        if content.is_empty() {
    ...
    async fn find_candidates(&self, _limit: usize) -> Vec<(Uuid, DateTime<Utc>)> {
        ...
        for node in all_nodes {
            ...
            // Only active memories (skip in lenient mode)
            if !lenient && node.status != MemoryStatus::Active {
                continue;
            }
            ...
            // Only compact important memories (configurable via env, default >= 3)
            // Skip importance check in lenient mode
            let min_importance: i32 = std::env::var("KNOWWHERE_CONSOLIDATION_MIN_IMPORTANCE")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(3);
            ...
            // Only compact nodes with substantial content (configurable via env, default > 500)
            let min_content_len: usize = std::env::var("KNOWWHERE_CONSOLIDATION_MIN_CONTENT_LEN")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(500);
