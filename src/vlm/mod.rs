...
impl SummaryContext {
    ...
    pub fn prompt_template(&self) -> &'static str {
        match self {
            SummaryContext::Summary => "Extract the key decision or fact:\n\n{content}",
            SummaryContext::Overview => {
...
impl VlmModel {
    ...
    pub fn base_url(&self, config: Option<&VlmConfig>) -> String {
        match self {
            VlmModel::Gpt5Nano | VlmModel::Gpt4oMini => "https://api.openai.com".to_string(),
    ...
    pub fn timeout_secs(&self) -> u64 {
        match self {
            ...
            VlmModel::Ollama | VlmModel::Custom => 180, // was 60 — M1/8GB needs more time for 2GB models
        }
    }
}
...
mod worker {
    ...
    impl VlmWorker {
        ...
        #[deprecated(
            since = "0.2.0",
            note = "Truncation causes information loss. Use LocalSummarizer or configure VLM."
        )]
