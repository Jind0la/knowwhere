...
impl LocalSummarizer {
    ...
    ///
    /// Uses OLLAMA_URL env var (default: http://127.0.0.1:11434)
    /// Uses OLLAMA_SUMMARIZER_MODEL env var (default: llama3.2)
    pub fn new() -> Result<Self> {
        let ollama_url =
            std::env::var("OLLAMA_URL").unwrap_or_else(|_| "http://127.0.0.1:11434".to_string());
        let model =
            std::env::var("OLLAMA_SUMMARIZER_MODEL").unwrap_or_else(|_| "llama3.2".to_string());
    ...
    pub async fn summarize(&self, text: &str) -> Result<String> {
        // Unified prompt — handles both decision-heavy technical content
        // and narrative/persona content (preferences, facts, life changes).
        // The LLM auto-detects content type and produces the right summary.
        let prompt = format!(
        ...
        let summary = summary.trim_matches('"').trim_matches('\'').to_string();
    ...
    pub async fn summarize_with_length(
        ...
        let prompt = format!(
            "Analyze this conversation and extract key information. \
             ...
             - \"summary\": 2-3 sentence narrative summary covering the main topics, \
               preferences expressed, and any changes mentioned\n\n\
             - \"claims\": array of {{\"claim\", \"reason\"}} objects\n\n\
             For each notable piece of information:\n\
             - \"claim\": the specific fact, preference, decision, or change — \
               name exact activities, technologies, people, places\n\
             - \"reason\": the context, motivation, or evidence behind it\n\n\
             Include: preferences (likes/dislikes), life changes (stopped/started), \
             background facts, relationships, opinions, and decisions. \
             Be specific — 'stopped podcasting about music' not 'changed hobby'.\n\n{}",
            text
        );
...
impl TieredSummarizer {
    ...
    pub async fn summarize_for_tier(
        ...
        match tier {
            crate::memory::types::ContextTier::Summary => self.summarize_l0(text).await,
            crate::memory::types::ContextTier::Overview => self.summarize_l1(text).await,
            crate::memory::types::ContextTier::Raw => Ok(SummaryResult {
                text: text.to_string(),
...
mod tests {
    ...
    async fn test_summarize_deterministic() {
        ...
        let result1 = summarizer
            .summarize(text)
            .await
            .expect("first summarization failed");
        let result2 = summarizer
            .summarize(text)
            .await
            .expect("second summarization failed");
