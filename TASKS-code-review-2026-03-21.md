...
**Commit:** `e2182f2` — `fix: constant-time API key comparison (prevents timing attack)`
...
**Commit:** `4bd5c98` — `fix: exponential decay model (Ebbinghaus curve) with halflife_hours parameter`
...
**Commit:** `da43722` — `fix: AppState test fixture and node_type → memory_type in tests`
...
`rrf_fuse()` existiert in `in_memory.rs` — nutzt `k=60`, state-of-the-art.

---

## 📋 Aktuelle Übersicht

| Task | Status | Aufwand | Fix-Commit |
|------|--------|---------|------------|
| CRIT-001 Timing-Angriff | ✅ Erledigt | 30min | e2182f2 |
| CRIT-002 Rate-Limiting | ✅ Erledigt | 2h | 2a0d58e, 32d5023 |
| CRIT-003 PostgreSQL | 🔴 Offen | ~1 Tag | — |
...
| LOW-004 RRF | ✅ Erledigt | — | rrf_fuse() vorhanden |
